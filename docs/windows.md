...
Release実行ファイルの文字入力とコマンド実行、設定再読込、混在DPIモニター
追従、分割境界、最大化、最小化、復元、正常終了をまとめて確認できます。
...
することも確認してからプロセスを閉じます。利用者の設定ファイルは変更せず、
一時設定とマーカーは正常終了後または自動後処理後に削除します。標準出力と標準エラーは
