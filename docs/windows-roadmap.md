...
| 3 | ウィンドウ機能 | 完了 | 100% |
| 4 | 分割ペイン | 完了 | 100% |
| 5 | Windows GUI仕上げ | 進行中 | 約80% |
| 6 | リリース品質 | 一部完了 | 約30% |

全体進捗は約84%と見積もる。現在地はフェーズ5の後半である。
...
| タブ操作 | 95% | 選択、終了、追加、キー・ホイール移動、ドラッグを実装済み |
| 分割境界GUI | 85% | ヒット判定、方向カーソル、ドラッグ、最小サイズ制約を実装済み |
| GUI状態表示 | 70% | フォーカス、タイトル、分割ズーム状態に対応済み |
...
1. 任意のタブ名変更を実装するか、コマンドタイトルのみを仕様として確定する。
2. 必要に応じて右クリックメニューを追加する。
3. 分割境界の色、太さ、ハイコントラスト表示を最終調整する。
4. タイトルバーとタブバーの外観、DPI、キーボード操作、アクセシビリティを
   最終調整する。
...
多数タブ時の表示と操作は完了した。現在の次工程は、タブ名の仕様確定と
必要な編集操作である。

1. macOS/Linux版のタブ名変更アクションと設定を確認する。
2. コマンドタイトルとの優先順位と、任意名を保持する期間を定義する。
3. Windows版に必要な編集入口とキーボード操作を実装する。
4. 外観、DPI、アクセシビリティ調整へ進む。
