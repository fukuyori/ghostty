...
| 3 | ウィンドウ機能 | 完了 | 100% |
| 4 | 分割ペイン | 完了 | 100% |
| 5 | Windows GUI仕上げ | 進行中 | 約85% |
| 6 | リリース品質 | 一部完了 | 約30% |

全体進捗は約85%と見積もる。現在地はフェーズ5の後半である。
...
| 項目 | 進捗 | 状態 |
|---|---:|---|
| ネイティブタイトルバー | 75% | ダークモード、背景色、文字色、タブ名とのタイトル同期を実装済み |
...
2. 分割境界の色、太さ、ハイコントラスト表示を最終調整する。
3. タイトルバーとタブバーの外観、DPI、キーボード操作、アクセシビリティを
   最終調整する。
4. 複数モニター、最大化、最小化、復元時のGUI追従を確認する。
...
1. 分割境界を通常時とハイコントラスト時の双方で判別できるよう調整する。
2. タイトルバーとタブバーのDPI追従を複数DPI環境で確認する。
3. キーボード操作とアクセシビリティ情報を確認・補完する。
