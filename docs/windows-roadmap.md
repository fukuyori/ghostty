...
全体進捗は約97%と見積もる。現在地はフェーズ5の最終確認と、並行して進める
フェーズ6の文書整備である。
...
5. Windowsシェルの実操作とスリープ復帰時のGUI追従を確認する。
...
- 専用プロセスと一時設定で文字入力・コマンド実行、設定再読込、複数
  ...
  ウィンドウ状態回帰スクリプト
- ウィンドウ状態回帰を反復数または時間で継続実行し、各回の結果とログパスを
  JSONへ逐次保存する耐久試験スクリプト
- Alt+Tab、タスクバー、スナップ、複数ウィンドウのWindowsシェル実操作を案内し、
  合否・環境・ログをJSONへ逐次保存する手動受け入れ試験スクリプト
...
- Alt+TabでGhostty固有のアイコンが表示されない問題を修正し、Windowsシェルの
  手動受け入れ試験を再実行する。スナップ、選択、タスクバー操作は確認済み。
- 診断起動ログを実際の初期化失敗や描画復旧調査で継続利用し、必要な項目を補う。
- 144 DPI・192 DPIとスリープ復帰を試験する。長時間稼働用の反復・時間指定
  スクリプトと20反復の基準結果は確認済みであり、数時間規模の実行結果を取得する。
...
4. 最大化、最小化、復元、混在DPIモニター移動、分割ペインを含む正常終了は
   自動回帰確認が完了した。`scripts/test-windows-shell.ps1` でWindowsシェルの
   実操作を記録し、別途スリープ復帰を確認する。
...
| Windowsシェル | スナップ・Alt+Tab・タスクバー適格性 | 実操作5項目合格、Alt+Tabの固有アイコンだけ不合格 |
| 復帰 | スリープ | 未確認 |
