...
全体進捗は約96%と見積もる。現在地はフェーズ5の最終確認と、並行して進める
フェーズ6の文書整備である。
...
- 専用プロセスと一時設定で文字入力・コマンド実行、設定再読込、Windows
  シェル適格性、混在DPIモニター移動、タブバーと分割境界の追従、最大化、
  最小化、復元、正常終了を検査するウィンドウ状態回帰スクリプト
...
- Windows標準のスナップ、Alt+Tab、タスクバー操作を実機で回帰確認する。
- 診断起動ログを実際の初期化失敗や描画復旧調査で継続利用し、必要な項目を補う。
- 144 DPI・192 DPI、スリープ復帰、長時間稼働を試験する。
...
4. 最大化、最小化、復元、混在DPIモニター移動、分割ペインを含む正常終了は
   自動回帰確認が完了した。Windowsシェルの実操作とスリープ復帰を確認する。
...
| 設定 | 設定再読込 | 一時設定によるタブバーの非表示・再表示をRelease版で自動確認済み |
...
| Windowsシェル | スナップ・Alt+Tab・タスクバー適格性 | スタイル・所有関係・DWM状態を自動確認済み、実操作未確認 |
| 復帰 | スリープ | 未確認 |
